...
describe Robot do
  ...
  describe "#heal_shield" do
    it "increases shield health" do 
      @robot.wound!(25)
      @robot.heal_shield(Battery::PROVIDED_HEALTH)
      expect(@robot.shield).to eq(Robot::MAX_SHIELD)
    end

    it "doesn't increase shield health over 50" do
      @robot.heal_shield(Battery::PROVIDED_HEALTH)
      expect(@robot.shield).to eq(Robot::MAX_SHIELD)
    end
  end
